...
const Home = () => {
  ...
  return (
    <div className="px-10 lg:px-0 max-w-7xl mx-auto">
      ...
      <div className="px-6 mt-20">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 p-8 mt-12 bg-blue-50 rounded-xl">
          {ourProducts.map((items, index) => (
            <div
              key={index}
              className="flex gap-4 items-center bg-white py-20 px-10 rounded-xl hover:scale-[1.03] hover:shadow-xl ease-in-out duration-300 cursor-pointer"
            >
              <span className="text-blue-600">{items.img}</span>
              <h3 className="text-[#1A3967] text-3xl font-semibold">
                {items.name}
      ...
      {/*Redirect Section*/}
      <div className="flex flex-col md:flex-row justify-between items-center px-6 py-10 mt-20 gap-8 md:gap-12 bg-gradient-to-r from-[#094486] to-[#4682D4] text-white rounded-xl">
